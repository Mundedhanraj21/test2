...
ICollection editorial_categories_CreateDataSource() {
    ...
    {
    // Create a simple COUNT(*) query from the original without ORDER BY
      editorial_categories_sCountSQL = "SELECT COUNT(*) FROM [editorial_categories] e " + sWhere;
    }
    ...
    int PageTemp = (result != null && result != DBNull.Value) ? Convert.ToInt32(result) : 0;
