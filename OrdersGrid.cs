...
ICollection Orders_CreateDataSource() {
	...
	//-------------------------------
	//-------------------------------
// Build ORDER BY statement (SECURE)
//-------------------------------

// Define a whitelist of allowed sort columns (match these to your SQL SELECT aliases or full names)
string[] allowedColumns = {
    "o.order_id", "o.item_id", "o.member_id", "o.quantity",
    "i.name", "m.member_login"
};

// Default order clause
sOrder = " ORDER BY o.order_id ASC";

// Read and validate user-supplied sorting input
string sortColumn = Utility.GetParam("FormOrders_Sorting");
string sortDir = "ASC"; // default

if (!IsPostBack && !string.IsNullOrEmpty(sortColumn))
{
    // Only accept allowed columns
    if (Array.Exists(allowedColumns, col => col.Equals(sortColumn, StringComparison.OrdinalIgnoreCase)))
    {
        ...
        ViewState["SortDir"] = "ASC"; // Default direction on first sort
    }
}

if (ViewState["SortColumn"] != null)
{
    string column = ViewState["SortColumn"].ToString();
    string direction = (ViewState["SortDir"]?.ToString()?.ToUpper() == "DESC") ? "DESC" : "ASC";

    // Final validation before use
    if (Array.Exists(allowedColumns, col => col.Equals(column, StringComparison.OrdinalIgnoreCase)))
    {
        sOrder = $" ORDER BY {column} {direction}";
    }
}
    ...
    {
        PageTemp = Convert.ToInt32(result);
    }
