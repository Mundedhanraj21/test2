...
private static readonly Dictionary<string, string> AllowedSortColumns = new Dictionary<string, string>
{
    { "member_login", "m.member_login" },
...
private static readonly List<string> AllowedSortDirections = new List<string> { "ASC", "DESC" };
...
public void ApplySorting()
{
    ...
    string sortColumn = "member_login"; // Default column
    string sortDirection = "ASC"; // Default direction
    
    // Check if ViewState["SortColumn"] is a valid column in the allowed list
    if (ViewState["SortColumn"] != null && AllowedSortColumns.ContainsKey(ViewState["SortColumn"].ToString()))
    {
        sortColumn = ViewState["SortColumn"].ToString();
    }
    
    // Check if ViewState["SortDir"] is a valid direction
    if (ViewState["SortDir"] != null && AllowedSortDirections.Contains(ViewState["SortDir"].ToString()))
    {
        sortDirection = ViewState["SortDir"].ToString();
    }

    // Apply the safe sorting order
    sOrder = $"ORDER BY {AllowedSortColumns[sortColumn]} {sortDirection}";
}
	...
	object result = ccommand.ExecuteScalar();
    int PageTemp = (result != DBNull.Value && result != null) ? Convert.ToInt32(result) : 0;
