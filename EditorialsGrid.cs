...
ICollection editorials_CreateDataSource() {
	...
	if (ViewState["SortColumn"] != null && ViewState["SortDir"] != null)
    {
       sOrder = " ORDER BY " + ViewState["SortColumn"].ToString() + " " + ViewState["SortDir"].ToString();
    }
     else
    {
    // Default sorting column and direction if ViewState is null
    sOrder = " ORDER BY article_title ASC";
    }
	...
	//-------------------------------
	  ...
	  L = editorials_sSQL + sWhere + sOrder;

// Construct the count SQL only if it's empty
   if (string.IsNullOrEmpty(editorials_sCountSQL)) {
    // Find the part of the SQL after SELECT and before FROM
    int selectIndex = editorials_sSQL.IndexOf("SELECT ", StringComparison.OrdinalIgnoreCase);
    int fromIndex = editorials_sSQL.IndexOf("FROM ", StringComparison.OrdinalIgnoreCase);
    
    // Get the part between SELECT and FROM (the columns)
    string columnsPart = editorials_sSQL.Substring(selectIndex + "SELECT ".Length, fromIndex - (selectIndex + "SELECT ".Length));

    // Construct the count query by replacing columns with "COUNT(*)"
    editorials_sCountSQL = "SELECT COUNT(*) " + editorials_sSQL.Substring(fromIndex);

    // Remove any ORDER BY clause if present in the count query
    int orderByIndex = editorials_sCountSQL.IndexOf("ORDER BY", StringComparison.OrdinalIgnoreCase);
    if (orderByIndex >= 0) {
        editorials_sCountSQL = editorials_sCountSQL.Substring(0, orderByIndex);
    }
}
    ...
    object result = ccommand.ExecuteScalar();  // Execute the scalar query and get the result

// Default value for PageTemp
    int PageTemp = 0;  

// Check if the result is not null and if it can be parsed into an integer
   if (result != null && int.TryParse(result.ToString(), out PageTemp))
   {
    // If successfully parsed, PageTemp will hold the integer value
    }
   else
    {
    // If result is null or cannot be parsed to an integer, handle it gracefully
    PageTemp = 0;  // Default value when there's no valid result
    }
